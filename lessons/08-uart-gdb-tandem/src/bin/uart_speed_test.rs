//! # UART Speed Test
//!
//! Tests maximum UART throughput at various baudrates.
...
fn main() -> ! {
    ...
    // Default baudrate is 115200 - we'll test multiple speeds
    ...
    writeln!(uart, "BOOT|version=speed_test_1.0.0|chip=ESP32-C6").ok();
    writeln!(uart, "STATUS|baudrate=115200|packet_size=64").ok();
    ...
    loop {
        ...
        if timestamp_ms % 10 == 0 {
            ...
            // Write binary packet using UART write()
            uart.write(bytes).ok();
